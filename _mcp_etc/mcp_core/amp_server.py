...
def maps_weather(city: str, date: str) -> Dict[str, Any]:
    ...
    try:
        ...
        formated_forecasts = {"城市": forecasts[0]["city"]} 
        for forecast in forecasts[0]["casts"]:
            ...
            for key, name in {
                ...
                if forecast.get(key):
                    formated_forecasts[name] = forecast[key]
...
def maps_bicycling_by_address(origin_address: str, destination_address: str, origin_city: Optional[str] = None, destination_city: Optional[str] = None) -> Dict[str, Any]:
    """使用地址规划两地之间的自行车路线。除非您有特殊原因需要使用坐标，否则建议使用此工具。

    参数：
        origin_address：起点地址（例如“北京市朝阳区阜通东大街6号”）
        destination_address：终点地址（例如“北京市海淀区上地十街10号”）
        origin_city：可选的起点城市名称，用于提高地理编码准确性
        destination_city：可选的终点城市名称，用于提高地理编码准确性

    返回：
        路线信息，包括距离、持续时间和逐向导航指示。
        考虑桥梁、单行道和道路封闭情况。支持最长 500 公里的路线。
    """
    try:
...
def maps_bicycling(origin_coordinates: str, destination_coordinates: str) -> Dict[str, Any]:
    ...
    参数：
        origin_coordinates：起点坐标，格式为“经度,纬度”（例如“116.434307,39.90909”）
        destination_coordinates：终点坐标，格式为“经度,纬度”（例如“116.434307,39.90909”）

    返回：
        路线信息，包括距离、持续时间和逐向导航指示。
        考虑桥梁、单行道和封闭道路。支持最长 500 公里的路线。
    """
    try:
        response = requests.get(
